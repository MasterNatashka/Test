...
1. первая функция, проверка подключения и доступности камеры;
...
4. четвёртая функция, вывод видеопотока с каждой камеры в одном окне
...
Разработка программы (прим.: с алгоритмом): TMA_mk3
Разработать программу с параллельной разработкой алгоритма, выполняющую задачу:
1. первая функция проверки подключения камеры и повторного (прим.: раз в 5 секунд) опроса камеры (прим.: проверка на подключение камеры);
2. вторая функция, вывод видеопотока с ip камеры без детектирования объектов;
3. третья функция, измерения времени методами: 2, 3, 5 (прим.: Энциклопедия Харриса/Python -> https://wiki.yandex.ru/homepage/otdel-razvitija-texnologijj/jenciklopedija-xarrisa/python/ )
алгоритм программы должен выполнять задачу следующими методами:
1. treading - многопточность;
2. asyncio - асинхронное выполнение функций (прим.: задач)
...
# Функция проверка подключения и доступности камеры, и захват видеопотока с каждой камеры в отдельном потоке через функцию run_threads()
def video_stream(num_thread, url, window_name="Видеопоток с IP_камеры"):
    ...
    if not cap.isOpened():
        print(f"Ошибка: Не удалось открыть источник видео: {url}")
    #else:
...
if __name__=="__main__":
    ...
    print(f"Время выполнения всех потоков {times_end()} секунд", count_totalcadr)
